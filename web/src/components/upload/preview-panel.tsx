  ...
  return (
    <AnimatePresence mode="wait">
      {file && file.status === "ready" && (
        ...
        >
          {/* Thumbnail hero */}
          <div className="mx-auto flex h-48 w-36 flex-shrink-0 items-center justify-center overflow-hidden rounded-lg border border-border bg-surface-2 shadow-sm sm:mx-0">
            {file.thumbnail ? (
          ...
          <div className="flex min-w-0 flex-1 flex-col gap-3">
            ...
            <dl className="grid grid-cols-2 gap-x-4 gap-y-2 text-xs">
              {file.metadata?.title && (
                <div className="col-span-2 min-w-0">
                  ...
                  <dd className="truncate text-muted" title={file.metadata.title}>
                    {file.metadata.title}
                  </dd>
                </div>
              )}
              {file.metadata?.author && (
                <div className="col-span-2 min-w-0">
                  ...
                  <dd className="truncate text-muted" title={file.metadata.author}>
                    {file.metadata.author}
                  </dd>
                </div>
              )}
            ...
            <div className="mt-1 flex flex-wrap items-center gap-2">
              ...
              {file.metadata?.isEncrypted && (
                <span className="inline-flex items-center gap-1.5 rounded-full bg-warning-soft px-2.5 py-1 text-xs font-medium text-warning-foreground">
